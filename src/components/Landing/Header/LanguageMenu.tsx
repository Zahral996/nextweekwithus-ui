    ...
    <Menu as="div" className="relative inline-block text-left">
      <div>
        <Menu.Button className="inline-flex justify-center items-center w-full rounded-md py-2 bg-white text-sm font-medium hover:bg-gray-50">
          <img src={selectedLanguage.flag} alt={`${selectedLanguage.label} flag`} className="w-5 h-5 mr-2" />
        ...
        {languages.map((language) => (
          <Menu.Item key={language.code}>
            {({ active }) => (
              ...
              >
                <img src={language.flag} alt={`${language.label} flag`} className="w-5 h-5 mr-2" />
                {language.label}
              </button>
            )}
          </Menu.Item>
